...
const App = () => {
  ...
  const save = (arr) => {
    localStorage.setItem('starred-repos', JSON.stringify(arr))
  }

  const handleStarRepo = (repo) => {
    let starredList = [...starred, repo]
    let checkStar = starred.find(r => r.id === repo.id)

    if (!checkStar) setStarred(starredList)

    save(starredList)
  }

  const handleUnstarRepo = (repo) => {
    let starredList = starred.filter(item => item.id !== repo.id)
  ...
  // check starred items
  useEffect(() => {
    const getStarred = JSON.parse(localStorage.getItem('starred-repos'))
    if (getStarred) setStarred(getStarred)
  }, [])
  ...
  return (
    <>
      ...
      <main className="max-w-xl mx-auto">
        ...
        {starred.length > 0 && (
          <section className="my-8">
            <h4 className="text-xl font-bold mb-6">Estrelados {`(${starred.length})`}</h4>
            ...
            <ul className="flex flex-col gap-4">
              {starred.map(repo => (
                <Repository
                  ...
                  data={repo}
                  star={true}
                  handleStar={() => handleUnstarRepo(repo)}
                />
              ))}
            </ul>
          </section>
